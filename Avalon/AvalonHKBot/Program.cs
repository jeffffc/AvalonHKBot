...
namespace AvalonHKBot
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.Title = $"AvalonHKBot - Connected to {Bot.Me.FirstName} (@{Bot.Me.Username} | {Bot.Me.Id}) - Version {version.ToString()}";
            ...
            foreach (var m in typeof(Commands).GetMethods())
            {
                foreach (var a in m.GetCustomAttributes(true))
                {
                    if (a is Attributes.Command cmd)
                    {
                        ...
                        Bot.Commands.Add(new Models.Command(cmd.Trigger, cmd.AdminOnly, cmd.GlobalAdminOnly, cmd.DevOnly, cmd.GroupOnly, method));
                    }
        ...
        private static void UpdateConsole()
        {
            ...
            while (true)
            {
                ...
                foreach (Avalon game in games)
                {
                    table.AddRow(game.Id.ToString(), game.Phase.ToString(), game.Phase == Avalon.GamePhase.InGame ? game.NowAction.ToString() : "------", game.Players.Count().ToString());
                }
